...
<div class="card p-3">
    ...
    <h6 class="">
        <dt>ชื่อหน่วยงาน : <?= Html::encode($model->unit_name) ?></dt>
    </h6>
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <a href="#" data-toggle="tooltip" data-placement="top" title="จำนวนผู้ดูแลเต็มแล้ว!" class="disabled"><span
                    class="tag tag-info" style="cursor: pointer;display: inline-block;font-size: 10px;"><i
                        class="fa fa-plus" aria-hidden="true"></i> ผู้ดูแลหน่วย</span></a>
            <?php else: ?>
            <a onclick="window.open('index.php?r=users/create&unitid=<?=$model->unit_id;?>');" href="#"><span
                    class="tag tag-info" style="cursor: pointer;display: inline-block;font-size: 10px;"><i
                        class="fa fa-plus" aria-hidden="true"></i> ผู้ดูแลหน่วย</span></a>
            <?php endif ?>
            ...
            <a href="#" data-toggle="tooltip" data-placement="top" title="จำนวนผู้ใช้งานเต็มแล้ว!"
                class="disabled"><span class="tag tag-warning text-dark"
                    style="cursor: pointer;display: inline-block;font-size: 10px;"><i class="fa fa-plus"
                        aria-hidden="true"></i> ผู้ใช้งานหน่วย</span></a>
            <?php else: ?>
            <a onclick="window.open('index.php?r=users/create_users&unitid=<?=$model->unit_id;?>&unitname=<?=$model->unit_name;?>');"
                href="#"><span class="tag tag-warning text-dark"
                    style="cursor: pointer;display: inline-block;font-size: 10px;"><i class="fa fa-plus"
                        aria-hidden="true"></i> ผู้ใช้งานหน่วย</span></a>
            <?php endif ?>

            <?php $checkundercover = Yii::$app->db->createCommand("SELECT COUNT(*) FROM undercover WHERE unitid = '".$model->undercover_limit."'")->queryScalar(); ?>
            <?php  if ($checkundercover>=$model->undercover_limit): ?>
            <a href="#" data-toggle="tooltip" data-placement="top" title="จำนวนสายข่าวเต็มแล้ว!" class="disabled"><span
                    class="tag tag-secondary" style="cursor: pointer;display: inline-block;font-size: 10px;"><i
                        class="fa fa-plus" aria-hidden="true"></i> สายข่าวหน่วย</span></a>
            <?php  else: ?>
            <a onclick="window.open('index.php?r=undercover/create_undercover&unitid=<?=$model->unit_id;?>&unitname=<?=$model->unit_name;?>');"
                href="#"><span class="tag tag-secondary"
                    style="cursor: pointer;display: inline-block;font-size: 10px;"><i class="fa fa-plus"
                        aria-hidden="true"></i> สายข่าวหน่วย</span></a>
            <?php  endif ?>

            <a data-toggle="modal" data-target="#manageform" href="#" data-unit_name="<?=$model->unit_name;?>"
                data-unit_id="<?=$model->unit_id;?>" class="manageform"><span class="tag tag-cyan"
                    style="cursor: pointer;display: inline-block;font-size: 10px;"><i class="fa fa-check-square"
                        aria-hidden="true"></i> จัดการฟอร์ม</span></a>
        </div>
