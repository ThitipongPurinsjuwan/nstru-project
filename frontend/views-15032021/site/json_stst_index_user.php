...
if ($type=='countusers') {
	...
	$data = array();
		...
		$useAll = Yii::$app->db->createCommand("SELECT COUNT(log_id) AS sum FROM `user_log_usaged` WHERE username = '".$_SESSION['user_id']."'")->queryScalar();

		$useday = Yii::$app->db->createCommand("SELECT COUNT(log_id) AS sum FROM `user_log_usaged` WHERE username = '".$_SESSION['user_id']."' AND create_date = '".$date_d."'")->queryScalar();
		$usemonths = Yii::$app->db->createCommand("SELECT COUNT(log_id) AS sum FROM `user_log_usaged` WHERE username = '".$_SESSION['user_id']."' AND create_date LIKE '".$date_y."-".$date_m."-%'")->queryScalar();
		$useyear = Yii::$app->db->createCommand("SELECT COUNT(log_id) AS sum FROM `user_log_usaged` WHERE username = '".$_SESSION['user_id']."' AND create_date LIKE '".$date_y."%'")->queryScalar();
