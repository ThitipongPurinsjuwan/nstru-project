...
?>
        <a class="btn-question goDoSomething" id="desc-model" data-id-desc="<?=$desc_modal['id'];?>" data-toggle="modal"
            data-target=".bd-modal-manual"><i class="fa fa-question" data-toggle="tooltip" data-placement="top" title="คู่มือการใช้งาน
"></i></a>
    </h4>
...
?>
            ...
            <p class="text-right">
            <?= Html::a('<i class="fe fe-plus" data-toggle="tooltip" title="" data-original-title="fe fe-plus"></i> เพิ่มหน่วยงาน', ['create'], ['class' => 'btn btn-success mb-3']) ?>
            </p>
            <?php
$columns = 2;
$cl = 12 / $columns;
	...
	<b>พิกัด (${data[i].lat}, ${data[i].lon})</b>`);
                ...
                $(document).on('click', '.loadmapsearch', function() {
                    ...
                    function loaddata_search(name, id) {
                        var url_json = "index.php?r=unit/map_marker&type=search&unitname=" + name +
                            "&unitid=" + id;
            ...
            <form id="frmValues">
                <div class="modal-header">
                    <h5 class="modal-title">
                        <dt>จัดการแบบฟอร์มหน่วยงาน <span id="show_unitname"></span></dt>
                    </h5>
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                </div>
...
$(document).ready(function() {
    $(document).on('click', '.manageform', function() {
        ...
        $.ajax({
            url: "index.php?r=unit/manage_eform&type=show&auth=Authenticator=>2ffa459adcc37176dbf93a82addf61dc<?=date("Ymd");?>&unitid=" +
                unitid,
            method: "GET",
            dataType: "json",
            ...
            success: function(data) {
                ...
                if (data.length > 0) {
                    ...
                    show_data_eform +=
                        `<input type="hidden" name="unit_id_check" id="unit_id_check" value="${unitid}">`;
    ...
    $(document).on('click', '.save_eform_unit', function() {
        ...
        $.ajax({
            url: "index.php?r=unit/manage_eform&type=update_data&auth=Authenticator=>2ffa459adcc37176dbf93a82addf61dc" +
                convertDate(new Date()) + "&unitid=" + unitid,
            data: frmValues,
            type: 'post',
            dataType: 'json',
            // processData: false,
            ...
            success: function(data) {
                ...
                // if (data.output==1) {
                $("#show_error").html(
                    '<div class="alert alert-success alert-dismissible fade show" role="alert"><strong><i class="fas fa-check-circle" aria-hidden="true"></i></strong> บันทึกข้อมูลสำเร็จ <button type="button" class="close" data-dismiss="alert" aria-label="Close"></button></div>'
                    );
