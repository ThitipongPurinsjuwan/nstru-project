	...
	foreach ($query as $cols){
		...
		foreach ($data_main[0]['fieldGroup'] as $col){
			...
			if ($col['type']=='select') {
				if ($col['templateOptions']['model']!=null) {
					$show .= "<b>".$col['templateOptions']['placeholder']."</b> : ".substr($dta[$col['key']],0,300)."..."."<br>";
				}else{
					$show .= "<b>".$col['templateOptions']['placeholder']."</b> : ".substr($dta[$col['key']],0,300)."..."."<br>";
				}
				...
				$content = $dta[$col['key']];
				$new_content = iconv_substr($content,0,100,'UTF-8');
				$show .= "<b>".$col['templateOptions']['placeholder']."</b> : ".$new_content."..."."<br>";
		...
		if ($dta['approve'] != '') {
			if (count($dta['approve'])==count($object_approve_template[0])) {
				$status = '<span class="tag tag-success"> <i class="fe fe-check-circle" data-toggle="tooltip" title="อนุมัติแล้ว" data-original-title=""></i> </span>';
				$select_approve = '<i class="fe fe-check-circle" data-toggle="tooltip" title="อนุมัติแล้ว" data-original-title=""></i>';
			}else{
				$text_status = $object_approve_template[0][$dta['approve'][$ii]['step']];
				$status = '<span class="tag tag-primary">'.'<i class="fe fe-file-text" title="ประเมินข่าว"></i>'.'</span>';
				$select_approve = '<i class="fe fe-file-text" title="ประเมินข่าว"></i>';
			}
			...
			if(count($dta['request_information'])>=0){
				...
				$status = '<span class="tag tag-warning"><b><i class="fa fa-question-circle-o" title="ต้องการข้อมูลเพิ่มเติม"></i> </b></span> : <span class="badge badge-light text-dark" style="">'.$text_request_information.'</span>';
				$select_approve = '<i class="fa fa-question-circle-o" title="ต้องการข้อมูลเพิ่มเติม"></i>';
			}else{
				$select_approve = '<i class="fe fe-alert-circle" title="รอการอนุมัติ"></i>';
			}

			}else{
				// $select_approve = '<i class="fe fe-alert-circle" title="รอการอนุมัติ"></i>';
			
				$status = '<span class="tag tag-danger" style="    background-color: #db2828 !important;"><i class="fe fe-alert-circle" title="รอการอนุมัติ"></i></span> <br>'.$text_request_information;
			}
			// $status = '<span class="tag tag-danger" style="    background-color: #db2828 !important;"><i class="fe fe-alert-circle" title="รอการอนุมัติ"></i></span> <br>'.$text_request_information;
		}
		...
		$data[] = array(
			...
			"link"=> '<a class="btn btn-light btn-sm" href="index.php?r=eform-data/approve_status&id='.$cols['eform_data_id'].'" style="white-space: nowrap;"><i class="fas fa-eye"></i> </a>'
		);
		$i++;
	}
