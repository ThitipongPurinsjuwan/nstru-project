...
<div class="eform-template-form">
	...
	<div class="row">
        ...
        <!-- <div class="card-header bg-secondary text-white">
            ...
            <div class="form-group col-md-12 mt-4">
            	...
            	<?= Html::submitButton('บันทึกและแสดงตัวอย่าง', ['class' => 'btn btn-success savesort']) ?>
    ...
    <div class="modal fade exampleModalSelect" tabindex="-1" role="dialog" id="exampleModalSelect" aria-labelledby="myLargeModalLabel" aria-hidden="true">
...
$(document).on('click', '.show_data', function(){
	...
	if (eform_array['type']=='input') {
		...
		check_id = '3';
		selecttype1 = type_input_text(1,eform_array['templateOptions']['maxlength']);
		select_type_textarea = type_select_textarea(1,eform_array['templateOptions']['type_textarea']);
        console.log(eform_array['templateOptions']['type_textarea']);
...
var typeinput_eform = '<div class="col-md-6"><div id="show_select_model">'+selecttype1+'</div><div id="show_select_type_textarea">'+select_type_textarea+'</div><div id="show_option_value">'+selecttype2+'</div></div>';
...
<br>
<b>ขนาดช่องกรอก</b> (className) <a class="btn-question" data-toggle="modal" data-target=".exampleModalSelect"><i class="fa fa-question" data-toggle="tooltip" data-placement="top" title="การใช้งาน Layout – Grid
"></i></a><br>
...
$(document).on('click', '.type_select', function(){
	...
	$("#show_select_type_textarea").html('');
...
$(document).on('click', '.type_text', function(){
	type_input_text(2,'');
	$(".show_key").css('display', 'block');
	$(".show_placeholder").css('display', 'block');
...
$(document).on('click', '.type_textarea', function(){
    $(".show_key").css('display', 'block');
    $(".show_placeholder").css('display', 'block');
    $(".show_classname").css('display', 'none');
    $("#show_required_eform").css('display', 'block');
...
function type_select_textarea(type,val){
	var checked_val = '';
	if (val!=null) {
		checked_val = val;
	}

	var checked_type_textarea1 = '';
	var checked_type_textarea2 = '';

  if (checked_val>0) {
    checked_type_textarea1 = '';
    checked_type_textarea2 = 'checked';
}else{
   checked_type_textarea1 = 'checked';
   checked_type_textarea2 = '';
}

var show_input_text = '';
console.log(checked_val);
show_input_text += `<br>
<b>ประเภทช่องกรอก</b> <a class="btn-question" data-toggle="modal" data-target=".showdetailtextarea"><i class="fa fa-question" data-toggle="tooltip" data-placement="top" title="รูปแบบช่องกรอกรายละเอียด
"></i></a><br>
<input type="radio" class="select_type_textarea" name="type_textarea" value="0" ${checked_type_textarea1}> ไม่มีรูปแบบ<br>
<input type="radio" class="select_type_textarea" name="type_textarea" value="1" ${checked_type_textarea2}> มีรูปแบบ`;

if (type==1) {
  return show_input_text;
} else {
  $("#show_select_type_textarea").html(show_input_text);
}
}
...
$(document).on('click', '.saveupdate', function(){
	...
	if (type_check_input === undefined || type_check_input === null) {
        ...
        var type_textarea = $("input[type='radio'].select_type_textarea:checked").val();
        if (type_textarea == '') {
            check_error_eform = 1;
        }else{
            check_error_eform = 0;
...
if (check_error_eform==0) {
  if (key!='' && placeholder!='') {
    if (key=="files") {
        check_error_eform = 1;
        text_alert = "ไม่อนุญาตให้ตั้งชื่อคอลัมน์ว่า 'files'";
    }else{
        check_error_eform = 0;
    }
} else {
...
<div class="modal fade showdetailtextarea" tabindex="-1" role="dialog" id="showdetailtextarea" aria-labelledby="myLargeModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">

			<!-- Modal Header -->
			<div class="modal-header">
				<h4 class="modal-title">รูปแบบช่องกรอกรายละเอียด</h4>
				<button type="button" class="close" data-dismiss="modal">&times;</button>
			</div>

			<!-- Modal body -->
			<div class="modal-body">
				<label for="noformat">ไม่มีรูปแบบ</label>
				<textarea class="form-control" id="noformat" rows="4"></textarea>

                <label for="format">มีรูปแบบ</label>
                <textarea class="summernote" id="format" rows="3"></textarea>
            </div>
