...
<div class="file-upload-list-view">
    <? #=$model->text_extract?>
    <h4><span class="badge badge-primary"><?= Html::encode($this->title) ?></span></h4>
    <div class="row clearfix">
        <div class="col-xl-12 col-lg-12 col-md-12">
            <div class="card card-info">
                <div class="card-body ribbon">
                    <p align="right">
                        <button class="btn save-btn btn-primary" onclick="showfiles('<?=$model->file_name;?>','<?=$model->id;?>','<?=$model->bucket;?>');"><i class="fa fa-download"></i> โหลดไฟล์</button>
                        <!-- --> <a  onClick="extractText('<?=$model->id?>','<?=$model->file_name;?>','<?=$model->bucket;?>');" href="#extract"   class="btn  btn-primary" ><i class="fa fa-refresh"></i> ประมวลผลใหม่</a> 

                        <!-- href="index.php?r=file-upload-list/manage_words&id=<?=$model->id;?>" -->
                        <a href="index.php?r=file-upload-list/words_cut&id=<?=$model->id;?>" class="btn  btn-primary" ><i class="fa fa-cut"></i> วิเคราะห์ข้อมูล</a>
                        <button class="btn save-btn btn-primary deldata" data-file-id="<?=$model->id;?>" data-name-file="<?=$model->file_name;?>" data-name-bucket="<?=$model->bucket;?>"><i class="fa fa-trash"></i> ลบ</button>
                    ...
                    <?= DetailView::widget([
                        ...
                        'attributes' => [
                            ...
                            [
                                ...
                                {
                                    //return '<span class="badge badge-primary">'.$model->origin_file_name.'</span>';
                                    return ''.$model->origin_file_name.'';
                                }
                            ],
                            /* [
...
<script>
  ...
  // $('.show-text').html(''+showdata.join(""));  // ตัดคำ Elasticsearch
});
