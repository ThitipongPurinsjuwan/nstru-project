<?php if (isset($_SESSION['user_id'])): ?>
	...
	<?php if ($type=='show'){
		...
		$new_str = preg_replace('~[\\\\/:*?"<>|]~', ' ', $text1);
			//<button class="btn btn-primary btn-sm extractText" data-file_id="'.$row['id'].'" data-file_name="'.$row['file_name'].'" data-bucket="'.$row['bucket'].'" data-text_extract="'.$new_str.'" data-toggle="modal" data-target="#myModal" type="button" style="white-space: nowrap;">ประมวลผล</button>
			$text_extract = '<div class="btn-group">
			
			 <a class="btn btn-secondary btn-sm" href="index.php?r=file-upload-list/words_cut&amp;id='.$row['id'].'" title="view"><i class="fa fa-eye"></i></a> 
			 <button class="btn save-btn btn-danger deldata btn-sm" data-file-id="'.$row['id'].'" data-name-file="'.$row['file_name'].'" data-name-bucket="'.$row['bucket'].'"><i class="fa fa-trash"></i></button></div>';
			...
			$resultArray[] = array(
				...
				'file_id' => $row['id'],
				'file_name' => $link_file,  // 
				'bucket' => '<center><i class="fa fa-file-word-o"></i></center>', //$row['bucket'],
